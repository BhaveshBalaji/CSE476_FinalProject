...
def build_answers(questions: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    ...
    for idx, question in enumerate(questions, start=1):
        ...
        # In case if the model didnt provide any answer, I set it to "not answered"
        if not real_answer or not real_answer.strip():
            real_answer = "not answered"
        answers.append({"output": real_answer})

        # added progress for every 100 questions to enure its running fine.
        if idx % 100 == 0:
